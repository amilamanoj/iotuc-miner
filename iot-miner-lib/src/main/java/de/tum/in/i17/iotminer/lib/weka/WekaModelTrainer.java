...
public class WekaModelTrainer implements Trainer{
    ...
    public void trainStep1() {
        try {
            WekaClassifier classifier = new WekaClassifier(
                    new File(WekaClassifier.class.getResource("/supervised/data/step1").toURI()), "weka-model-s1.txt");
            classifier.train();
    ...
    public void trainStep2() {
        try {
            WekaClassifier classifier = new WekaClassifier(
                    new File(WekaClassifier.class.getResource("/supervised/data/step2").toURI()), "weka-model-s2.txt");
            classifier.train();
            classifier.process("enabling access to for millions more in asia feedly smartcity", "");
            classifier.saveModel();
        } catch (Exception e) {
